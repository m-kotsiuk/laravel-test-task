            ...
            <form action="{{ route('employees.update', $employee) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div style="width: 300px">
                        <img src="{{ $employee->photo  }}" alt="" class="img-fluid mb-4 js-photo-img" />
                    </div>
                    <div class="input-group">
                        <div class="custom-file ">
                            <input type="file" name="photo_file" class="custom-file-input js-photo-input" id="employeeImage">
                ...
                <div class="form-group">
                    ...
                    >
                        @if(old('position_id', $employee->position_id))
                            <option value="{{ old('position_id', $employee->position_id) }}" selected>{{ App\Models\Position::find(old('position_id', $employee->position_id))->name }}</option>
                        @endif
                    </select>
                    <div class="text-danger">
                ...
                <div class="form-group">
                    ...
                    <select
                        ...
                        name="parent_id"
                    >
                        @if($employee->parent_id && old('parent_id', $employee->parent_id))
                            <option value="{{ old('parent_id', $employee->parent_id) }}" selected>{{ $employee->parent()->first()->name }}</option>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group date" id="employmentdate" data-target-input="nearest">
                        <input
                            ...
                            value="{{ old('employment_date', $employee->employment_date->format('d.m.y')) }}"
                        />
                ...
                <div class="row no-gutters px-2">
                    <div class="col-md-6">
                        <p><strong>{{ __('Created at') }}:</strong> {{  $employee->created_at->format('d.m.y') }}</p>
                        <p><strong>{{ __('Updated at') }}:</strong> {{  $employee->updated_at->format('d.m.y') }}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>{{ __('Admin created ID') }}:</strong> {{  $employee->admin_created_id }}</p>
                        <p><strong>{{ __('Admin updated ID') }}:</strong> {{  $employee->admin_updated_id }}</p>
                    </div>
                </div>
